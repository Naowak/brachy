...
# -*- coding: utf-8 -*-
# zone_influence.py
""" Gère les calculs et redimensionnement liés à la zone d'influence """
...
def get_domaine_minimal(sources, n_points, dimensions, maillage, zone_influence):
    """
    Retourne le domaine minimal à considérer (4 points formant un rectangle)
    Ce domaine est calculé en considérant les sources et leur zone d'influence
    On prend les 4 points les plus extremes pour réaliser ce rectangle
    """
    # Initialisation
    (lf, mf, nf) = n_points
    (Lx, Ly, Lz) = dimensions
...
def matrix_to_domaine(matrix, domaine):
    ...
    # (y_min, x_min) = domaine[0]
    # (y_max, x_max) = domaine[1]
...
def get_domaine_n_points(domaine, old_n_points):
    ...
    mf = (y_max - y_min) + 1
    nf = old_n_points[2]
    n_points = (abs(lf), abs(mf), abs(nf))

    return n_points


def get_domaine_dimensions(domaine, old_dimensions, old_maillage):
    """ Permet de convertir les dimensions actuelles vers les dimensions du domaine """
    # Recuperation des informations
    (x_min, y_min) = domaine[0]
    (x_max, y_max) = domaine[1]
    ...
    Lz = old_dimensions[2]
    dimensions = (abs(Lx), abs(Ly), abs(Lz))

    return dimensions


def get_domaine_sources(domaine, sources):
    """ Permet de convertir les coord des sources actuelles vers les coord vers le domaine """
    # Recuperation des informations
    (x_min, y_min) = domaine[0]
    (x_max, y_max) = domaine[1]
...
def get_domaine_HU_array(domaine, old_HU_array):
    """ Permet de convertir la matrice HU_array en gardant seulemlent les valeurs dans le domaine  """
    return matrix_to_domaine(old_HU_array, domaine)


def get_coord_domaine(domaine, maillage):
    """ Permet de récupérer les coordonnées exactes du domaine à l'aide du maillage """
    # Recuperation des informations
    (x_min, y_min) = domaine[0]
    (x_max, y_max) = domaine[1]    
    (maillage_x, maillage_y, maillage_z) = maillage
