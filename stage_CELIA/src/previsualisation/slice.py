...
class Slice:
    ...
    def initialize(self):
        ...
        try :
            ...
            #pour une raison inconnu, la lecture du dicom 
            #n'a pas donné la profondeur de chaque coupe
            #On va donc la chercher nous meme dans le DICOM
            for line in self.dicom_slice :
                line = str(line)
                if "Image Position (Patient)" in line :
                    self.position = abs(float(line.split("'")[-2]))
