#!/usr/bin/env python
# -*- coding: utf-8 -*-
# generate_multisource.py
""" Permet la génération des fichiers .don nécessaires au lancement des calculs avec KIDS """
...
def get_maillage(n_points, dimensions):
    ...
    xm = np.linspace(0 + taille_maille_x/2.0, Lx - taille_maille_x/2.0, abs(lf))
    ym = np.linspace(0 + taille_maille_y/2.0, Ly - taille_maille_y/2.0, abs(mf))
...
def ajouter_header(f, algorithme, n_points, dimensions):
...
def ajouter_source(f, groupe, type_particule, direction_M1, volume_sphere, spectre_mono, calculs_finaux):
    ...
    volume_shere : un sextuplet (x, y, z, rx, ry, rz) avec r_ le rayon de la sphère selon les axes
    ...
    res += "-frontiere direction_M1 " + str(direction_M1[0]) + " " + str(direction_M1[1]) + \
           " " +  str(direction_M1[2]) + "\n"
    res += "-frontiere volume_spheroid " + str(volume_sphere[0]) + " " + str(volume_sphere[1]) + \
...
def lancer_generation(filename, sources, n_points, dimensions, options, calculs_finaux=False):
    ...
    ajouter_header(f, algorithme, n_points, dimensions)
    ajouter_densite(f, densite_lu)

    # On ajoute les sources dans le fichier .don
    groupe = 1
    for source in sources:
        x1 = float(int(maillage_x[source[0]] * 1000))/1000
        x2 = float(int(maillage_y[source[1]] * 1000))/1000
        x3 = float(int(z*1000))/1000
        volume_sphere = (x1, x2, x3, rx, ry, rz)
        ajouter_source(f, groupe, "g", direction_M1, volume_sphere, spectre_mono, calculs_finaux)
        groupe += 1
