...
def get_appartenance_contourage(n_points, maillage, contourage):
    ...
    # my_points = np.zeros([2, 2])
    # my_points[0] = (250, 250)
    # print(contourage_path.contains_points(my_points))
    # a = [list(points[i]) for i in range(len(points)) if contourage_path.contains_points([points[i]])]
    # print(a)
    ...
    for x in range(lf):
        for y in range(mf):
            ...
            # if appartenance_contourage_2D[x, y] :
            #     print((x,y))
            indice_1D += 1
