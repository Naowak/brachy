...
########################### Metadata ###########################
    ...
    def get_DICOM_npoints(self):
        ...
        """
        lf = int(self.slices_info.Columns)
        mf = int(self.slices_info.Rows)
        nf = 1
    ...
    def get_DICOM_dimensions(self):
        ...
        Lx = float(self.slices_info.PixelSpacing[0]) * lf * 0.1
        Ly = float(self.slices_info.PixelSpacing[1]) * mf * 0.1
        Lz = 1
...
###################### Generation des fichiers pour le calcul de dose ######################
    ...
    def generate_DICOM_previsualisation(self, slice_id, working_directory, options):
        ...
        if densite_lu == 1:
            ...
            #SOLUTION TEMPORAIRE PAS BELLE
            domaine = [[domaine[0][1], domaine[0][0]], [domaine[1][1], domaine[1][0]]]
            domaine_HU_array = get_domaine_HU_array(domaine, HU_array)
    ...
    def generate_DICOM_calculs_finaux(self, slice_id, working_directory, options):
        ...
        directory = working_directory + "/slice_" + str(slice_id).zfill(3) + "/calculs_finaux/"
        filename = directory + "config_KIDS_calculs_finaux.don"
